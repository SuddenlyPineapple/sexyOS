...
enum Process_state {NEW,READY,RUNNING,WAITING,TERMINATED,ZOMBIE,ORPHAN};// stany procesu
class PCB
{
public:
	...
	std::vector<PageTableData> page_table;//vector stronic - tak chcesz kasperski?
...
public:
	PCB proc;
	void fork(PCB* proc,const std::string name/*,tutaj memory mangment z przydzielaniem pamieci */ );//dodaje kopieprocesu(dzieciaka) procesu do drzewa
	void fork(PCB* proc, const std::string name/*,tutaj memory mangment z przydzielaniem pamieci */,std::string file_name);//to samo co wyzej tylko z nazwa pliku
	void fork(PCB * proc, const std::string name/*,tutaj memory mangment z przydzielaniem pamieci */, std::vector<std::string> file_names);//to samo co wyzej tylko z vectorem plików
